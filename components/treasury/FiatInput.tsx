    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <Label className="text-h3 text-foreground">Fiat Currencies</Label>
        </div>
        <Button
          type="button"
          variant="outline"
      ...
      {fiatHoldings.length === 0 ? (
        <div className="text-center py-8 bg-muted rounded-panel border border-dashed border-knob-silver dark:border-knob-silver-dark">
          <p className="text-body text-muted-foreground">No fiat currencies added yet</p>
        </div>
      ) : (
          ...
          {fiatHoldings.map((fiat) => (
            <div key={fiat.id} className="flex items-end gap-3">
              <div className="flex-1">
                <Label htmlFor={`fiat-currency-${fiat.id}`} className="text-caption">
                  Currency
                </Label>
                <select
              ...
              <div className="flex-1">
                <Label htmlFor={`fiat-amount-${fiat.id}`} className="text-caption">
                  Amount
                </Label>
                <Input
              ...
              <Button
                ...
                className="h-9 w-9"
                aria-label={`Remove ${fiat.currency}`}
              >
