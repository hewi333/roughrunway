...
server.tool(
  "build_runway_model",
  `Build a crypto treasury runway model on roughrunway.com and return a shareable link.
...
If only a total burn is given, pass it as a single number; headcount = team × $15,000/month.`,
  {
    ...
    stablecoins: z
      ...
      .optional()
      .describe(
        'Stablecoin balances as NAME:AMOUNT pairs, comma-separated. e.g. "USDC:1500000,USDT:200000"'
      ),
    volatile: z
      .string()
      .optional()
      .describe(
        'Volatile crypto holdings as TICKER:QTY:PRICE:TIER, comma-separated. e.g. "ETH:50:3500:major,NEX:100000000:0.08:native"'
      ),
    burn: z
      .string()
      .optional()
      .describe(
        'Monthly burn — single number ("150000") or category:amount pairs ("headcount:105000,infrastructure:18000")'
      ),
    team: z
      .number()
      .int()
  ...
  async ({ name, stablecoins, volatile, burn, team, months }) => {
    ...
    if (stablecoins) params.set("stable", stablecoins);
    if (volatile)    params.set("volatile", volatile);
    ...
    try {
      ...
      return {
        content: [{ type: "text" as const, text: `Network error reaching RoughRunway: ${err}` }],
        isError: true,
      };
    }
    ...
    if (!res.ok) {
      ...
      return {
        content: [{ type: "text" as const, text: `RoughRunway returned an error (${res.status}): ${body}` }],
        isError: true,
      };
    }

    const { shareUrl, modelName } = (await res.json()) as {
      shareUrl: string;
      modelName: string;
    };
