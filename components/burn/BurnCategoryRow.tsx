...
import AdjustmentRow from "@/components/burn/AdjustmentRow";

interface BurnCategoryRowProps {
  category: BurnCategory;
}

export default function BurnCategoryRow({ category }: BurnCategoryRowProps) {
  const { model, updateModel } = useRoughRunwayStore();
  const { burnCategories } = model;
  const [isExpanded, setIsExpanded] = useState(false);

  const updateCategory = (updates: Partial<BurnCategory>) => {
    const updatedCategories = burnCategories.map((c) =>
  ...
  const deleteCategory = () => {
    const updatedCategories = burnCategories.filter((c) => c.id !== category.id);
    updateModel({ burnCategories: updatedCategories });
  };
